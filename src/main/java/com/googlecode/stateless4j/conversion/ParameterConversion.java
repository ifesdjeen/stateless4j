...
public class ParameterConversion {
  public static Object Unpack(Object[] args, Class<?> argType, int index) throws Exception {
    ...
    if (args.length <= index)
      throw new Exception(String.format(ParameterConversionResources.ArgOfTypeRequiredInPosition, argType, index));

    Object arg = args[index];

    if (arg != null && !argType.isAssignableFrom(arg.getClass()))
      throw new Exception(String.format(ParameterConversionResources.WrongArgType, index, arg.getClass(), argType));
  ...
  public static void Validate(Object[] args, Class<?>[] expected) throws Exception {
    if (args.length > expected.length)
      throw new Exception(String.format(ParameterConversionResources.TooManyParameters, expected.length, args.length));
